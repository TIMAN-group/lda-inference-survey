...
\begin{figure}[h]
  ...
  \caption{Graphical model for the factorized variational distribution for
    ``smoothed LDA'' in \cite{Blei:2003:LDA}.}
...
\begin{equation}
  \lambda_{i,r} = \beta_r + \sum_{j=1}^M \sum_{t=1}^{N_j} \pi_{j,t,r}
  \mathbf{1}(w_{j,t} = r)
\end{equation}

This bears a striking resemblance to the update for $\phi_{i,r}$ in the
variational EM algorithm given in the previous section's
Equation~\ref{eqn:varemphi}, but with the addition of the prior's
pseudo-counts $\beta_r$.
